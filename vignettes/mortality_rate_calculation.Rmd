...
## Data
For England, to inform the cause-specific central rates of death in one year intervals of age and period, data on annual counts of deaths and mid-year population sizes was requested from the Office for National Statistics (in the form of mortality micro-data). Data is processed from 2001 (being the first year for which estimates by Index of Multiple Deprivation quintile are available) to the latest available. The data is used to calculate and forecast the central rates of death, and from period life-tables to estimate the years of life lost to death from each cause. Causes are defined by groupings of ICD-10 codes ([see the information on the relative risks of disease](https://stapm.gitlab.io/data.html#Relative_risks_of_disease)). Death rate and life-table calculations are stratified by sex and IMD quintile population subgroups.       

### Death counts
Death counts data for England are stratified as follows:  
...
### Population counts
Mid-year population size data for England:  
