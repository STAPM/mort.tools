...
# and to use the data to calculate Scottish national death rates for tobacco-related diseases

# Prepare the death rates from tobacco related diseases for Scotland
...
tob_icd10_data <- read.csv(paste0(path, "/tobalc_icd10_lookups.csv"))
setDT(tob_icd10_data)

# Load the death counts by age, sex, IMD quintile, year and cause
deaths_data <- mort.tools::ReadData(path, country = "Scotland", supp_data = tob_icd10_data)
...
# you only want the deaths for tobacco related conditions - 
...
write.table(deaths_data, paste0("vignettes/Scotland_mortality_calculations/outputs/tob_death_rates_scot_national_2008-2021_v1_", Sys.Date(), "_mort.tools_", ver, ".csv"),
  row.names = F, sep = ",")
